          ...
          ${activeConvo && otherParty ? `
            <div class="p-3 border-b border-slate-200 flex items-center gap-3">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="text-xs text-emerald-600 flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-emerald-500"></span> Online</div>
              </div>
...
function startChat(otherId) {
  ...
  if (!convo) {
    convo = { id: uid('conv'), schoolId: 'sch_brightlights', participants: [me, otherId], messages: [] };
...
function view_announce_shared(role) {
  const announcements = DB.get('announcements').sort((a, b) => b.timestamp.localeCompare(a.timestamp));
  ...
  return `
    ...
    ${announcements.length === 0 ? emptyState({ title: 'No announcements', body: 'Announcements will appear here.', icon: 'bell' }) : `
      <div class="space-y-3">
        ${announcements.map(a => `<div class="card p-5">
          <div class="flex items-start gap-3 mb-3">
            ...
            <div class="flex-1">
              ...
              <p class="text-xs text-slate-500">${fdate(a.timestamp, { long: true })} · ${a.audience === 'all' ? 'Everyone' : a.audience.charAt(0).toUpperCase() + a.audience.slice(1)}</p>
            </div>
            <span class="badge badge-${a.audience === 'all' ? 'info' : 'neutral'}">${a.audience}</span>
          </div>
...
function saveAnnouncement() {
  ...
  let recipients = [];
  if (audience === 'all' || audience === 'parents') recipients.push(...DB.get('parents'));
  if (audience === 'all' || audience === 'teachers') recipients.push(...DB.get('teachers'));
