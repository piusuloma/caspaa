...
function seedDatabase() {
  ...
  const assignments = [
    { id: uid('asn'), schoolId, classId: 'cls_jss1', subjectId: 'sub_math', teacherId: 'tch_adamu', title: 'Algebra Practice - Set 3', description: 'Solve exercises 1-15 from page 42 of your textbook. Show all working steps.', dueDate: daysAhead(3), createdAt: daysAgo(2), submissions: [{ studentId: 'stu_009', submittedAt: daysAgo(2), text: 'Please see my working in the attached image.', file: { name: 'algebra_working.svg', type: 'image/svg+xml', data: _asnImgData, size: '14 KB' }, grade: 68, gradedAt: daysAgo(1), feedback: 'Good attempt Ade! You made a sign error in question 7 — when you move a term across the equals sign the sign must flip. Also, you need to show all working steps for questions 11 and 12 — answers alone will not get full marks. Please revise and resubmit.', markStatus: 'needs_revision', resubmissionRequested: true, returned: true, returnedAt: daysAgo(1), marginalComments: [{ id: 'pin_seed1', x: 38.5, y: 44.0, text: 'Sign error here — should be −3x, not +3x. Remember: moving to the other side flips the sign.', createdAt: daysAgo(1) }, { id: 'pin_seed2', x: 71.0, y: 67.5, text: 'Show all steps — you jumped straight to the answer here. Write each step on a new line.', createdAt: daysAgo(1) }] }] },
  ...
  const schoolRoles = [
    { id: 'role_proprietor', schoolId, name: 'Proprietor',     description: 'Full system control. Cannot be deleted.', system: true,  permissions: ['*'],                                                                            color: '#e8483d' },
    { id: 'role_principal',  schoolId, name: 'Principal',      description: 'Academic + administrative oversight, no finance.', system: true,  permissions: ['students','staff','academic','attendance','results','discipline','admissions','alumni','sickbay','communications'], color: '#fd5f54' },
    { id: 'role_vp',         schoolId, name: 'Vice Principal', description: 'Same as Principal, secondary signatory.',          system: false, permissions: ['students','staff','academic','attendance','results','discipline','admissions','sickbay','communications'], color: '#fd5f54' },
    ...
    { id: 'role_parent',     schoolId, name: 'Parent',         description: 'View own child(ren) only.',                         system: true,  permissions: ['own_children','own_fees','messaging'],                     color: '#e8483d' }
  ];
  ...
  schools.forEach(s => {
    if (!s.branding) s.branding = {
      primaryColor: s.id === 'sch_brightlights' ? '#fd5f54' : (s.id === 'sch_horizon' ? '#c2352b' : '#0f766e'),
